...
- Runtime assets: ttachments/ stores downloaded files by thread ID, exports/ stores CSV outputs, .env carries secrets, and credentials.json/	oken.json support Gmail OAuth.
...
- Recent history leans on conventional prefixes (eat:, ix:); prefer imperative, scope-limited summaries such as eat: add agent sweep flag.
...
- Never commit secrets (.env, credentials.json, 	oken.json); generated exports and attachments should remain gitignored.
